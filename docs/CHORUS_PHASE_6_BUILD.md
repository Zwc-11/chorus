# Chorus — Phase 6 Build Instruction (Real-World Validation)

> Build-ready spec. Companion to the Phase 1–5 build docs.
> Goal: stop testing Chorus on Chorus, and point it at **real coding agents on real
...
| **B. The real run** | Run a real agent ×N on real tasks until a cliff appears; the LangSmith trace of a real Chorus bug | `ANTHROPIC_API_KEY` (or other), Docker for SWE-bench eval, ~\$20–200 for a small validation (not the \$1–2k headline run), a LangSmith account | **No — user-gated (keys + budget)** |
...
1. Chorus drives **at least one real third-party agent** (not just `SwePatchAgent`) through
...
4. At least one **external artifact**: an upstream reproduction issue (linked) **or** a named
   user running Chorus on their agent.
...
6. **The LangSmith-MCP demo.** Point Chorus at LangSmith, pick a real Chorus bug, pull its
...
7. **File it.** Open an upstream reproduction issue on the agent repo with the trace + the
   `pass^k` evidence, or get one external user running Chorus on their agent. Link it in the
   README.
...
- [ ] LangSmith-MCP self-debug loop demonstrated on a real Chorus bug (Tier B).
...
> "…and gates CI on statistical regression. Pointed at <real agent> on <benchmark>, Chorus
