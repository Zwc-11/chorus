...
> Phase 6 deliverable (architecture.md §6). Chorus emits a run as an OpenTelemetry
> trace, the trace lands in LangSmith, and a coding agent pulls it back through the
> **LangSmith MCP server** to debug a real Chorus bug — the "write → trace → debug"
...
murmur run/trace ──emit gen_ai.* spans──▶ LangSmith project
        ...
        │                                        ▼
   Chorus source ◀──debug from the trace── coding agent + LangSmith MCP server
```
...
names, durations, `chorus.*` attributes) leave the machine; add `--capture-content`
...
## 3. Pull the trace and debug Chorus from it
...
new LangSmith trace. That round trip — Chorus's own bug, found and fixed *through its
...
- **Tier B (you run it):** the live export to a real LangSmith project and the actual
  MCP-driven debugging of a real Chorus bug — needs `LANGSMITH_API_KEY`, the `[otel]`
