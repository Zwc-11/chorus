# Murmur — Phase 1 Build Instruction (Observation)

> Build-ready spec. Companion to `MURMUR_DESIGN.md` and `MURMUR_PHASES.md`.
...
5. **Dogfood:** Murmur's tracer is pointed at a Murmur run, and you can read your own spans.
...
- **Namespacing:** standard fields go under `gen_ai.*`; Murmur-specific fields go under
  `murmur.*`. Never invent attributes inside the `gen_ai` namespace.
...
**6. Replay marking.** Spans served from the log in replay mode carry `murmur.replay=true`;
the UI renders a replay glyph so a replayed trace is distinct from a live one.

**7. Dogfood.** Instrument Murmur's own run; confirm you can read Murmur's spans in Phoenix.
...
| Event | Span name | `gen_ai.*` / `murmur.*` attributes |
|---|---|---|
| `run_started` / `run_finished` | `agent.run` | `gen_ai.operation.name=invoke_agent`, `murmur.run.id`, `murmur.trajectory.id`, status |
| `step_started` | `step {n} · {phase}` | `murmur.step.index`, `murmur.step.phase` (plan/act/reflect/verify) |
...
| `contract_check` | `contract.check` | `murmur.contract.step`, `murmur.contract.result` (Phase 4 writes more here) |
| failure (on the failed span) | — | `murmur.failure.class` (`tool_error` / `schema_mismatch` / `context_drift` / …) |
| replayed span (any) | — | `murmur.replay=true` |
...
- **Right — inspector.** The selected span's attributes shown **verbatim** as
  `gen_ai.*` / `murmur.*` key–value pairs (including `span_id` / `trace_id`). Error values
...
- [ ] `gen_ai.*` names exact; `murmur.*` for everything project-specific; semconv pinned.
- [ ] Trace UI: three regions, waterfall, inspector, all four states.
- [ ] Replay produces `murmur.replay=true` spans; UI marks them.
- [ ] Content-capture flag defaults off; test proves no content leaks when off.
- [ ] Dogfood: Murmur's own run is readable in Phoenix.
...
**1. Apply the four metric corrections** (paste-ready code is in `MURMUR_PHASES.md` §Phase 2):
