...
```bash
chorus fix-test --cmd "pytest tests/checkout/test_total.py -q"

chorus fix-issue --issue issue.md

chorus verify-pr --base main --head agent/fix-checkout

chorus eval-contract --task .chorus/tasks/bug_001.yaml --n 5
```

The first MVP command should be:

```bash
chorus fix-test --cmd "pytest tests/checkout/test_total.py -q" --budget 0.50
```
...
## 8. The Chorus Contract

A Chorus contract is a typed YAML file that can be generated automatically, edited by a human, and enforced by the runtime.
...
## 13. MVP Workflow: `chorus fix-test`
...
### 20.2 `chorus fix-test`

```bash
chorus fix-test --cmd "pytest tests/foo/test_bar.py -q"
```
...
### 20.3 `chorus contract create`

```bash
chorus contract create --from-test "pytest tests/foo/test_bar.py -q"
```
...
### 20.4 `chorus contract check`

```bash
chorus contract check .chorus/contracts/task.yaml
```
...
```bash
chorus run .chorus/contracts/task.yaml
```
...
### 20.7 `chorus verify-pr`

```bash
chorus verify-pr --base main --head agent/fix
```
...
```text
.chorus/runs/<run_id>/events.jsonl
.chorus/runs/<run_id>/contract.yaml
.chorus/runs/<run_id>/diff.patch
.chorus/runs/<run_id>/proof.md
.chorus/runs/<run_id>/report.html
```
...
```bash
chorus verify-pr --base main --head agent-branch
```
...
Make `empty_patch` a first-class category because it already appeared in the earlier Chorus run.
...
```bash
chorus fix-test --cmd "pytest tests/test_checkout.py -q" --budget 0.20
```
