...
class SubprocessSweEvaluator:
    def __init__(
        self,
        *,
        run_dir: Path | str = ".murmur/swebench",
        ...
        try:
            ...
            if exc.name == "resource":
                raise BenchDependencyMissing(
                    ...
                    "Windows Python. Run `murmur bench` from WSL/Linux, with Docker "
                    "Desktop WSL integration enabled."
                ) from exc
            raise BenchDependencyMissing(
                "swebench is not installed; `pip install 'murmur-ai-harness[bench]'` and ensure "
