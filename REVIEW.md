...
Full-codebase review pass, June 12, 2026. Every Python module in `murmur/` was
...
`murmur/adapters/sandboxes/local_worktree.py` — `_copy_tree` copied the
...
ignored `.murmur/`). The copy nested worktrees inside worktrees until the OS
rejected the path length. The default `.murmur/runs` masked this; `--out-dir
...
### 2. `python -m murmur.cli` silently did nothing (medium)
`murmur/cli.py` had no `if __name__ == "__main__"` block, so running the module
directly imported it and exited 0 — looking like success while doing nothing.
Fixed with a main guard; `python -m murmur.cli` now behaves like the installed
`murmur` command. (This is also what surfaced bug #1.)

### 3. Schema validator accepted booleans as integers/numbers (low)
`murmur/core/schema.py` — `isinstance(True, int)` is true in Python, so a step
...
`murmur/application/verifier.py` — `diff_lines` counted `+++`/`---` header
...
- `murmur/config/env.py`: repaired a mis-indented docstring line.
